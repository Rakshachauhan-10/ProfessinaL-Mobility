...
def generate_llama_response(text, query):
    prompt = f"Based on the following query, provide the following from the text given after it: '{query}'. Here is the text: {text}"
    result = llm.invoke(prompt)
    return result
...
def get_context(text, query):
    context = generate_llama_response(text, query)
    return context
